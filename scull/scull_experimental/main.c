 ...
 * Copyrihgt (C) 2020 Zoran Stojsavljevic
 *
...
/* The device will appear at /dev/scull_device using this value */
...
/*
 * @brief The LKM scull cleanup function
 * Similar to the initialization function, it is static. The __exit macro notifies that if
 * the code is used for a built-in driver (not a LKM) that this function is not required.
 */
static void __exit scull_exit(void)
{
	device_destroy(scull_class, MKDEV(scull_major, 0));	/* remove the device */
	class_destroy(scull_class);				/* remove the device class */
	scull_cleanup_module();
	printk(KERN_INFO "Scull: Removing the scull char LKM\n");
}
...
static int __init scull_init(void)
{
	...
	printk(KERN_INFO "scull: Initializing the scull char LKM\n");
	...
	if (scull_major) {
		...
		printk(KERN_INFO "scull: reallocated major number %d\n", scull_major);
	}
	if (result < 0) {
	...
	/* Register the device class - new section in scull_init() */
	...
	printk(KERN_INFO "scull: device class registered successfully\n");

#if 0
	/*
	 * Register the device drivers - done with the bash script
	 * this is why it is commented out
	 */
	scull_device = device_create(scull_class, NULL, MKDEV(scull_major, 0), NULL, DEVICE_NAME);

	/* Clean up if there is an scull device creation error */
